...
public class ComputerDaoImpl implements ComputerDao {
    ...
    private static final String BYID_REQUEST = "SELECT computer FROM Computer AS computer LEFT JOIN computer.company AS company WHERE computer.id=:id";
    ...
    @Override
    public Computer getById(Long id) throws ConnectionException {
        if (id < 1) {
            throw new IllegalArgumentException("Constraints are missing to getById");
        }
        Session session = getSession();
        Query<Computer> query = session.createQuery(BYID_REQUEST, Computer.class);
