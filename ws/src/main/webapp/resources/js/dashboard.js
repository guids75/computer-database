...
(function ( $ ) {
	...
	$.fn.toggleEditMode = function() {
		if($(".editMode").is(":visible")) {
			$(".editMode").hide();
			$("#editComputer").text(messages['label.edit']);
		}
		else {
			$(".editMode").show();
			$("#editComputer").text(messages['label.view']);
		}
		return this;
	};
...
(function ( $ ) {
	$.fn.deleteSelected = function() {
		if (confirm(messages['label.confirm'])) { 
...
//Function to order the list of computers
(function ( $ ) {
	$.fn.orderBy = function(e) {
		$('#orderForm input[name=order]').val(e);
		$('#orderForm').submit();
	};
}( jQuery ));
